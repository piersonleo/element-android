...
class NoticeEventFormatterVchat @Inject constructor(
    ...
    fun format(timelineEvent: TimelineEvent, isDm: Boolean): CharSequence? {
        val displayName = Utils.removeUrlSuffix(timelineEvent.senderInfo.disambiguatedDisplayName)

        return when (val type = timelineEvent.root.getClearType()) {
            EventType.STATE_ROOM_JOIN_RULES         -> formatJoinRulesEvent(timelineEvent.root, displayName, isDm)
            EventType.STATE_ROOM_CREATE             -> formatRoomCreateEvent(timelineEvent.root, isDm)
            EventType.STATE_ROOM_NAME               -> formatRoomNameEvent(timelineEvent.root, displayName)
            EventType.STATE_ROOM_TOPIC              -> formatRoomTopicEvent(timelineEvent.root, displayName)
            EventType.STATE_ROOM_AVATAR             -> formatRoomAvatarEvent(timelineEvent.root, displayName)
            EventType.STATE_ROOM_MEMBER             -> formatRoomMemberEvent(timelineEvent.root, displayName, isDm)
            EventType.STATE_ROOM_THIRD_PARTY_INVITE -> formatRoomThirdPartyInvite(timelineEvent.root, displayName, isDm)
            EventType.STATE_ROOM_ALIASES            -> formatRoomAliasesEvent(timelineEvent.root, displayName)
            EventType.STATE_ROOM_CANONICAL_ALIAS    -> formatRoomCanonicalAliasEvent(timelineEvent.root, displayName)
            EventType.STATE_ROOM_HISTORY_VISIBILITY ->
                formatRoomHistoryVisibilityEvent(timelineEvent.root, displayName, isDm)
            EventType.STATE_ROOM_SERVER_ACL         -> formatRoomServerAclEvent(timelineEvent.root, displayName)
            EventType.STATE_ROOM_GUEST_ACCESS       -> formatRoomGuestAccessEvent(timelineEvent.root, displayName, isDm)
            EventType.STATE_ROOM_ENCRYPTION         -> formatRoomEncryptionEvent(timelineEvent.root, displayName)
            EventType.STATE_ROOM_WIDGET,
            EventType.STATE_ROOM_WIDGET_LEGACY      -> formatWidgetEvent(timelineEvent.root, displayName!!)
            EventType.STATE_ROOM_TOMBSTONE          -> formatRoomTombstoneEvent(timelineEvent.root, displayName, isDm)
            EventType.STATE_ROOM_POWER_LEVELS       -> formatRoomPowerLevels(timelineEvent.root, displayName!!)
            EventType.CALL_INVITE,
            EventType.CALL_CANDIDATES,
            EventType.CALL_HANGUP,
            EventType.CALL_REJECT,
            EventType.CALL_ANSWER                   -> formatCallEvent(type, timelineEvent.root, displayName)
            ...
            Membership.INVITE -> {
                when {
                    eventContent.thirdPartyInvite != null -> {
                        ...
                        val threePidDisplayName = Utils.removeUrlSuffix(eventContent.thirdPartyInvite?.displayName) ?: ""
            ...
            Membership.JOIN   ->
                eventContent.safeReason?.let { reason ->
                    ...
                    if (event.isSentByCurrentUser()) {
                        ...
                        Timber.i("join: $senderDisplayName")
