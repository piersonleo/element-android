...
class FtueAuthLoginFragmentVchat @Inject constructor() : AbstractSSOFtueAuthFragment<FragmentLoginBinding>() {
    ...
    private var isConnectedToHomeServer = false
    ...
    private fun submitHomeServer(){
        val serverUrl = Constants.URL_DEV.ensureProtocol()
    ...
    private fun setupUi(state: OnboardingViewState) {
        views.loginFieldTil.hint = getString(when (state.signMode) {
            ...
            SignMode.Unknown, SignMode.SignIn -> R.string.login_signin_username_hint
        ...
        if (state.signMode == SignMode.SignInWithMatrixId) {
            ...
            val resId = when (state.signMode) {
                ...
                SignMode.Unknown, SignMode.SignIn -> R.string.login_connect_to
    ...
    private fun setupSubmitButton() {
        views.loginSubmit.setOnClickListener { if (isConnectedToHomeServer){
                submit()
            }else{
                submitHomeServer()
            }
    ...
    override fun updateWithState(state: OnboardingViewState) {
        ...
        if (state.signMode == SignMode.SignIn && !isConnectedToHomeServer){
            isConnectedToHomeServer = true
            submit()
        }
    }
